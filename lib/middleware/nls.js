...
function createNlsMiddleware(options)
{
  ...
  return function nlsMiddleware(req, res, next)
  {
    ...
    if (!/^(?:\/([a-z0-9\-]+))?\/([a-z0-9-_]+)\.js$/i.test(pathname))
    {
      return next();
    }
    ...
    const dashPos = pathname.indexOf('/', 1);

    if (dashPos !== -1)
    {
      locale = pathname.substring(1, dashPos);
    }
    ...
    {
      const nlsFile = dashPos === -1 ? pathname.substr(1) : pathname.substr(dashPos + 1);
    ...
    {
      jsonFile = options.jsonPath + pathname + 'on';
    }
